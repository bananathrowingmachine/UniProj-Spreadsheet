// Written by [redacted] for CS [redacted], September 2013
// Update by Profs [redacted] and [redacted], Fall 2021, Fall 2024
//     - Updated return types
//     - Updated documentation
// Finished by bananathrowingmachine, Oct 12, 2024
...
{
    /// <summary>
    ///   Stores a graph of cell dependencies to help calculate formulas and catch circular dependencies
    /// </summary>
    private DependencyGraph _varGraph = new();

    /// <summary>
    ///   Stores the contents and values of each cell, with the key to the dictionary being the name
    /// </summary>
    ...
    /// <summary>
    /// Constructs a spreadsheet using the saved data in the file referred to by the given filename.
    ...
    public Spreadsheet(string filename)
    {
        ...
        ReplaceSpreadsheet(File.ReadAllText(filename));
    }
    ...
    public void ReplaceSpreadsheet(string jsonData)
    {
        try
        {
            Spreadsheet? s = JsonSerializer.Deserialize<Spreadsheet>(jsonData);
            if (s == null) throw new SpreadsheetReadWriteException("Spreadsheet could not be deserialized");
            _cells = new();
            _varGraph = new();
            Changed = false;
            foreach (var cell in s._cells)
            {
    ...
    ///           Spreadsheet.Formula.FormulaFormatException is thrown.
    ///         </item>
    ///         <item>
    ...
    public IList<string> SetContentsOfCell(string name, string content)
    {
        ...
        if (content.Length != 0 && content[0] == '=')
            returnList = SetCellContents(name, new Formula(content[1..]));
        else
        {
            SheetWiper(name);
            Changed = true;
            returnList = double.TryParse(content, out var value)
    ...
    {
        _cells.Add(name.ToUpper(), new Cell(number, GetCellValue));
    ...
    private IList<string> SetCellContents(string name, string text)
    {
        if (text.Equals(string.Empty))
            ...
            _cells.Add(name.ToUpper(), new Cell(text, GetCellValue));
        return GetCellsToRecalculate(name).ToList();
    }
    ...
    {
        Dictionary<string, Cell> prevSheet = new Dictionary<string, Cell>(_cells);
        DependencyGraph prevGraph = _varGraph;
        SheetWiper(name);
        _cells.Add(name, new Cell(formula, GetCellValue));
        foreach (string variable in formula.GetVariables())
            _varGraph.AddDependency(variable, name);
        try
        {
            IList<string> returnList = GetCellsToRecalculate(name).ToList();
            Changed = true;
            return returnList;
        }
    ...
    /// <summary>
    ///   Removes the cell and all it's dependencies
    /// </summary>
    /// <param name="name"></param>
    private void SheetWiper(string name)
    {
        _cells.Remove(name);
        foreach (string dependee in _varGraph.GetDependees(name))
            _varGraph.RemoveDependency(dependee, name);
    }
    ...
    public void Save(string filename)
    {
        try
        {
            File.WriteAllText(filename, JsonStringData());
        }
        catch (Exception)
        {
            throw new SpreadsheetReadWriteException("Unable to save spreadsheet");
        }
    ...
    ///   value should be either a string, a double, or a Spreadsheet.Formula.FormulaError.
    /// </returns>
    /// <exception cref="InvalidNameException">
    ...
    {
        name = NameChecker(name);
        return _cells.TryGetValue(name, out var cell) ? cell.Value : string.Empty;
    }
    ...
    {
        return _varGraph.GetDependents(name);
    }
    ...
    /// <summary>
    ///  Does depth first search to find all dependents and any circular exceptions
    /// </summary>
    /// <param name="start">The start, to initially look down from and detect for circular exceptions</param>
    /// <param name="name">The current name to be looking down from</param>
    /// <param name="visited">Stores visited cells</param>
    /// <param name="changed">Stores sells to be changed</param>
    ...
    /// <summary>
    ///     Verifies the name of the variable is correct and capitalizes it.
    /// </summary>
    /// <param name="name">The name to be verified.</param>
    /// <returns></returns>
    /// <exception cref="InvalidNameException">Throws if the name is not a variable.</exception>
    private string NameChecker(string name)
    {
        var standaloneVarPattern = @"^[a-zA-Z]+\d+$";
        name = name.ToUpper();
